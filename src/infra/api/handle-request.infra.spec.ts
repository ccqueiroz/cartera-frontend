import { HandleRequestInfra } from "./handle-request.infra";
...
describe("Handle Request Infra", () => {
  let handleRequest: HandleRequestInfra;

  beforeEach(() => {
    handleRequest = new HandleRequestInfra();
  });

  it("should return success true with data when the function resolves", async () => {
    const result = await handleRequest.execute(() => Promise.resolve("data"));
