import { GetInitialDateFinalDateCurrentMonthHelper } from "./get-initial-date-final-date-current-month.infra";
...
describe("GetInitialDateFinalDateCurrentMonthHelper", () => {
  ...
  it("should return initial and final date for current month if no params passed", () => {
    mockDate("2024-06-15T00:00:00Z"); // Mock current date to June 15, 2024
  ...
  it("should fallback to current year if year is invalid", () => {
    ...
    const { initialDate, finalDate } = helper.execute("MAR", 2020); // year <= 2023 is invalid
